...
"""
This file defines the class ProbeFieldDiagnostic
...
class ProbeFieldDiagnostic(FieldDiagnostic):
    """
    Class that writes the value of the fields in a set of
    transverse planes that move in time.
    """
    def __init__(self, z0_planes, tmin, tmax, v_planes,
                period, fldobject, comm=None,
                fieldtypes=["E", "B", "J", "rho"], write_dir=None ) :
        """
        Initialize diagnostics that retrieve the fields in a set of
        moving planes (one file per plane).
        ...
        # the default directory ('./diags') in which diagnostics are written.
        ...
        # Find the z resolution and size of the diagnostic
        # (Needed to initialize metadata in the openPMD file)
        dz_slice = v_planes * self.fld.dt
        Nz = int( (tmax - tmin)/self.fld.dt ) + 1
        assert v_planes > 0
        ...
        # Create the list of Probes objects
        self.probes = []
        self.n_probes = len(z0_planes)
        for i in range(self.n_probes):
            probe = FieldProbe( self.write_dir, i, self.fld,
                                z0_planes[i], v_planes, tmin, tmax )
    ...
    def write( self, iteration ):
        ...
        self.store_probe_slices( iteration )

        # Every self.period, write the buffered slices to disk
        if iteration % self.period == 0:
            self.flush_to_disk()

    def store_probe_slices( self, iteration ):
        """
        ...
        for probe in self.probes:
            ...
            if ( (time > self.tmin) and (time < self.tmax) and \
                ...
                # and store the results into probe.slice_array
                # (when running on the GPU, probe.slice_array
                # is a device array)
                self.slice_handler.extract_slice( self.fld, self.comm,
                    probe.current_z, zmin, probe.slice_array )

                # Register probe.slice_array in the list of buffers
                # (when running on the GPU, copy the slice to the CPU)
                probe.register_slice( self.fld.dt )

    def flush_to_disk( self ):
        """
        Writes the buffered slices of fields to the disk

        Erase the buffered slices of the FieldProbe objects
        """
        # Loop through the probes and flush the data
        for probe in self.probes:

            # Compact the successive slices that have been buffered
            # over time into a single array, on each proc
            field_array, iz_min, iz_max = probe.compact_slices()
            ...
            if (self.rank==0) and (global_field_array is not None):
                ...
                self.write_slices( global_field_array, global_iz_min,
                    global_iz_max, probe, self.slice_handler.field_to_index)

            # Erase the memory buffers
            probe.buffered_slices = []
            probe.buffer_z_indices = []
    ...
    def write_slices( self, field_array, iz_min, iz_max, probe, f2i ):
        """
        For one given probe, write the slices of the
        ...
        f = self.open_file( probe.filename )

        field_path = "/data/%d/fields/" %probe.iteration
...
class FieldProbe:
    """
    Class that stores data relative to one given probe
    """
    def __init__(self, write_dir, i, fld, z0_plane, v_planes, tmin, tmax):
        """
        Initialize a FieldProbe

        Parameters
        ----------
        write_dir: string
            Absolute path to the directory where the data for
            this probe is to be written

        i: int
           Number of the file where this probe is to be written
        ...
        z0_plane: float
            The initial position at which the probe is

        v_planes: float
            The velocity of the probe

        tmin, tmax: float
            Time between which the diagnostics will be collected
        """
        # Deduce the name of the filename where this probe writes
    ...
    def update_current_output_positions( self, time ):
        """
        Update the positions of output for this probe
        ...
        for iz in self.buffer_z_indices[1:]:
            if iz != iz_old + 1:
                raise UserWarning(
                        ...
                        'The diagnostics may be inaccurate.')
                break
            iz_old = iz
...
class SliceHandler:
    """
    Class that extracts slices of the fields
    """
    def __init__( self ):
        """
    ...
    def extract_slice( self, fld, comm, z, zmin, slice_array ):
        """
        Fills `slice_array` with a slice of the fields at z
        ...
        z_staggered_gridunits = ( z - zmin - 0.5*dz )/dz
        ...
        if fld.use_cuda is False :
            ...
            # Extract a slice of the fields
            # at z, using interpolation, and store them in slice_array
